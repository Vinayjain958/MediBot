  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      content: "Hello! I am MediBot, your AI medical assistant. How can I help you today?"
    }
  ]);
        ...
        <div className={`p-4 border-t flex-shrink-0 z-10 transition-colors flex flex-col justify-end ${isDark ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50/50 border-slate-200'}`}>
          <div className="w-full">
            <form onSubmit={handleSubmit} className="relative flex items-end gap-2">
              <div className={`flex-1 flex items-end gap-2 rounded-xl border p-1 shadow-sm focus-within:ring-2 focus-within:ring-blue-500/50 transition-all ${
                ...
                <textarea
                  ...
                  placeholder="Describe symptoms or ask for advice... (Attachments disabled per instructions)"
                  className={`flex-1 max-h-48 min-h-[48px] py-3 px-4 resize-none bg-transparent outline-none text-sm w-full ${
            ...
            <div className={`text-center mt-3 text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
              MediBot is an AI assistant generating answers via Google Gemini 2.5 Flash. Not professional medical advice.
            </div>
          </div>
        </div>
      </main>
